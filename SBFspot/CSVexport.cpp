	...
	if (cfg->CSV_Export == 1)
	{
        ...
        else
        {
			...
			else
			{
				...
				{
					fprintf(csv, "sep=%c\n", cfg->delimiter);
					fprintf(csv, "Version CSV1|Tool SBFspot%s (%s)|Linebreaks CR/LF|Delimiter %s|Decimalpoint %s|Precision %d\n\n", cfg->prgVersion, OS, delim2txt(cfg->delimiter), dp2txt(cfg->decimalpoint), cfg->precision);
...
int WriteStandardHeader(FILE *csv, const Config *cfg, DEVICECLASS devclass)
{
	...
	char batt1[] = "||||Watt|Watt|Watt|Amp|Amp|Amp|Volt|Volt|Volt|Watt|kWh|kWh|Hz|hours|hours|Status|%|degC|Volt|Amp\n";
	char batt2[] = "|DeviceName|DeviceType|Serial|Pac1|Pac2|Pac3|Iac1|Iac2|Iac3|Uac1|Uac2|Uac3|PacTot|EToday|ETotal|Frequency|OperatingTime|FeedInTime|Condition|SOC|Tempbatt|Ubatt|Ibatt\n";
...
{	
	...
	char batt1[] = "|GridMs.W.phsA|GridMs.W.phsB|GridMs.W.phsC|GridMs.A.phsA|GridMs.A.phsB|GridMs.A.phsC|GridMs.PhV.phsA|GridMs.PhV.phsB|GridMs.PhV.phsC|GridMs.TotW|Metering.DykWh|Metering.TotWhOut|GridMs.Hz|Metering.TotOpTms|Metering.TotFeedTms|Operation.Health|Bat.ChaStt|Bat.TmpVal|Bat.Vol|Bat.Amp";
	char batt2[] = "|Analog|Analog|Analog|Analog|Analog|Analog|Analog|Analog|Analog|Analog|Counter|Counter|Analog|Counter|Counter|Status|Analog|Analog|Analog|Analog";
	char batt3[] = "|Watt|Watt|Watt|Amp|Amp|Amp|Volt|Volt|Volt|Watt|kWh|kWh|Hz|Hours|Hours||%|degC|Volt|Amp";
...
int ExportEventsToCSV(const Config *cfg, InverterData *inverters[], std::string dt_range_csv)
{
	...
	else
	{
		...
		#endif
		{
			...
			{
				fprintf(csv, "sep=%c\n", cfg->delimiter);
				fprintf(csv, "Version CSV1|Tool SBFspot%s (%s)|Linebreaks CR/LF|Delimiter %s\n\n", cfg->prgVersion, OS, delim2txt(cfg->delimiter));
			}
			if (cfg->CSV_Header == 1)
			{
